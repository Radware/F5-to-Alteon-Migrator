...
| AUTOMAP-1 | (feature, field request) **SNAT Automap** was flagged "address manually" - and a partner testing with a prospect found EVERY service in the config used it (18/18 virtuals) | **Converted to `pip mode egress`** (device usage: `mode [disable\|ingress\|egress\|address\|nwclss]`), which is Alteon's automap analog: the service source-NATs clients using the Proxy IP of the egress VLAN/port. **Live traffic proof (34.5.7):** with `/c/slb/pip/type vlan` + `/c/slb/pip/add <free-IP> <vlan>` + `pip mode egress`, the backend saw the PIP as the client and the one-armed return path worked - the same data-plane behavior as F5 automap. **Critical caveat found by the same test:** egress mode with an EMPTY PIP table does NOT NAT and traffic times out, so the conversion emits a REQUIRED-companion diagnostic quoting the exact `/c/slb/pip` commands (the converter cannot invent a free IP). The requester's other suggestion, `rtsrcmac`, is offered in the diagnostic as the no-NAT alternative (preserves client IPs - different behavior, not a default). |
