...
| AUTOMAP-1/2 | (feature, field request) **SNAT Automap** was flagged "address manually" - and a partner testing with a prospect found EVERY service in the config used it (18/18 virtuals) | **Converted to `pip mode egress` with the Proxy IP table auto-filled from the F5 FLOATING self-IPs.** Chain of live proofs on 34.5.7: (1) `pip mode egress` exists (`mode [disable\|ingress\|egress\|address\|nwclss]`) and with a per-VLAN PIP the backend saw the PIP as the client + the one-armed return path worked - automap's exact data plane; (2) egress mode with an EMPTY PIP table does NOT NAT and traffic times out - so emitting only the mode (as the field request literally asked) would break every service; (3) the device REFUSES its own interface IP as PIP ("The IP Address of Interface 1 conflicts with the Client NAT") - F5's self-IP sharing does not carry over; (4) **the FLOATING IP is ACCEPTED as PIP and NATs correctly under traffic** - and that is the very address F5 automap prefers, so the servers/firewalls keep seeing the same NAT source after migration. Configs with floats get the PIP table generated automatically (per VLAN, per RD split) + an informational note incl. an HA-review caveat; configs without floats (e.g. single-NIC) get a REQUIRED free-IP diagnostic quoting the exact commands, with `rtsrcmac` offered as the no-NAT alternative. |
| TMC-1 | (feature) **Source-match forwarding virtuals** (`ltm traffic-matching-criteria` + `net address-list`) were emitted as a "no direct Alteon equivalent" diagnostic | **Converted to an Alteon filter that matches the source against a network class.** `net address-list` becomes `/c/slb/nwclss <id>` (`type address`, `ipver v4`, one `network N/net subnet <ip> <mask> include` per entry) and the virtual becomes a filter with `sip <class>` (no `smask` - a class carries its own masks). Classes are emitted before the filters that reference them. Syntax discovered from the device itself (the filter menu documents sip/dip as "IP address **or network class**") and **validated live on 34.5.7 with a real customer config**: the class applied with all 10 addresses, the device reported "Network Class ... is associated to the following filter", and `filt cur` showed `sip Inside_ESA-Ties, dip any, proto any, action allow`. Destination address-lists, port-lists and missing lists each emit a targeted diagnostic instead of a wrong guess. |
| LIVE-22 | **`pbind ssl` is silently ignored by the device** - a virtual with F5 SSL persistence (`ltm persistence ssl`) was emitted with `pbind ssl`, which the CLI accepts without error while the persistence mode stays `disabled`. The migrated service lost stickiness with no warning anywhere | the keyword is **`sslid`** (device usage: `pbind clientip\|cookie\|sslid\|disable` on an SSL service, `clientip\|cookie\|disable` on a plain HTTP one). Now emits `pbind sslid` + `ptmout` on SSL-terminating services (https/ssl) and, on any other service type, omits it with a diagnostic instead of writing a line that does nothing. Verified live on 34.5.7: `pbind ssl` produced no change, `pbind sslid` applied and `cur` reported `pbind sslid ... ptmout 30 mins`. |
